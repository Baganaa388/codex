...
export const AdminContest: React.FC<Props> = ({
  contestId,
  authFetch,
  onBack,
}) => {
  ...
  const loadData = async () => {
    ...
    const [contestRes, problemsRes, contestantsRes] = await Promise.all([
      ...
      authFetch<{ rows: Contestant[]; total: number } | Contestant[]>(
...
function SettingsTab({
  ...
  useEffect(() => {
    const t = (contest as any)?.timeline;
    setTimeline(Array.isArray(t) && t.length > 0 ? t : DEFAULT_TIMELINE_ITEMS);
    setRegistrationFee((contest as any)?.registration_fee ?? 0);
  }, [contest]);
